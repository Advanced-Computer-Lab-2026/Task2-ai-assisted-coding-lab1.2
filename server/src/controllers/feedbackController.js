...
export async function getFeedbackSummary(req, res, next) {
  try {
    ...
    if (!result) return res.json({ eventCode, averageScore: 0, feedbackCount: 0 });
    res.json({ eventCode, averageScore: result.averageScore, feedbackCount: result.feedbackCount });
  } catch (err) { next(err); }
}
